...
func (f *Interface) readOutsidePackets(addr *udp.Addr, via interface{}, out []byte, packet []byte, h *header.H, fwPacket *firewall.Packet, lhf udp.LightHouseHandlerFunc, nb []byte, q int, localCache firewall.ConntrackCache) {
	...
	case header.Test:
		...
		case header.TestHostReply:
			...
			f.handleTestHostReply(hostinfo, uint64(time.Now().UnixNano()), m)
		...
		case header.TestRelayReply:
			...
			if from == f.myVpnIp {
				...
				if targetHI, err := f.hostMap.QueryVpnIp(target); err == nil {
					f.handleTestRelayReply(targetHI, uint64(time.Now().UnixNano()), m)
				}
			} else {
...
func (f *Interface) handleTestHostReply(hostinfo *HostInfo, nowNs uint64, m *NebulaTestHost) {
	if !f.lightHouse.GetRemoteAllowList().Allow(hostinfo.vpnIp, m.Ip) {
		hostinfo.logger(f.l).WithField("msg", m).Debug("lighthouse.remote_allow_list denied roaming")
		return
	}

	ip, ok := netip.AddrFromSlice(m.Ip)
	if !ok {
		hostinfo.logger(f.l).WithField("msg", m).Error("handleTestHostReply Failed addr from slice")
		return
	}

	val, ok := hostinfo.roamRemotes.Load(netip.AddrPortFrom(ip, uint16(m.Port)))
	if !ok {
		return
	}
	...
	// Should filter the except rtt
	roamRtt := nowNs - m.Time
	if state.SRtt != 0 {
		roamRtt = (state.SRtt*SRttAlphaPots + (SRttAlphaBasic-SRttAlphaPots)*roamRtt) / SRttAlphaBasic
	}

	state.LastRoamTime = m.Time
	state.SRtt = roamRtt
}

func (f *Interface) handleTestRelayReply(hostinfo *HostInfo, nowNs uint64, m *NebulaTestRelay) {
	val, ok := hostinfo.roamRemotes.Load(iputil.VpnIp(m.RelayIp))
	if !ok {
		return
	}
